...
class CreatePromoCodeCategoryFilterTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $faqTableName = 'promo_code_category_filter';
        $promoCodeTableName = 'promo_code';

        Schema::create($faqTableName, function (Blueprint $table) use ($faqTableName, $promoCodeTableName) {
            $table->id();

            $table->string('category_filter')->comment('Категория')->nullable();
            $table->string('category_filter_alias')->comment('Категория алиас')->nullable();

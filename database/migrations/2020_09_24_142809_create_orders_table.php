...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($ordersTableName, function (Blueprint $table) use ($ordersTableName, $usersTableName) {
            ...
            $table->decimal('promo_sum', 10, 2)->comment('Сумма промокода если применился')->default(0);
            ...
            $table->decimal('total', 10, 2)->comment('Конечная сумма заказа к оплате')->nullable();
            $table->decimal('certificate_sum', 10, 2)->comment('Сумма примененного сертификата')->nullable();
            $table->decimal('cost', 10, 2)->comment('Себестоимость заказа')->nullable();
            $table->string('shipping_method')->comment('Способ доставки')->nullable();
            $table->decimal('shipping_sum', 10, 2)->comment('Стоимость доставки')->nullable();
            ...
            $table->decimal('onlineDiscount', 10, 2)->comment('Скидка при онлайн оплате/рассрочке')->nullable();

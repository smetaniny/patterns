...
class CreatePromoCodeMetalFilterTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $faqTableName = 'promo_code_metal_filter';
        $promoCodeTableName = 'promo_code';

        Schema::create($faqTableName, function (Blueprint $table) use ($faqTableName, $promoCodeTableName) {
            $table->id();

            $table->string('metal_filter')->comment('Металл')->nullable();
            $table->string('metal_filter_alias')->comment('Металл алиас')->nullable();

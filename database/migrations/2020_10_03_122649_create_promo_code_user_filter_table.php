...
class CreatePromoCodeUserFilterTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        $faqTableName = 'promo_code_user_filter';
        $promoCodeTableName = 'promo_code';

        Schema::create($faqTableName, function (Blueprint $table) use ($faqTableName, $promoCodeTableName) {
            $table->id();

            $table->string('user_filter')->comment('Пользователь')->nullable();
            $table->string('user_filter_alias')->comment('Пользователь алиас')->nullable();
